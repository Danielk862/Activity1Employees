...
namespace Employee.Frontend.Components.Pages.Employees
{
    public partial class EmployeesIndex
    {
        ...
        private MudTable<EmployeeModel>? table = new();
        ...
        private async Task<TableData<EmployeeModel>> LoadListAsync(TableState state, CancellationToken cancellationToken)
        {
            ...
            var responseHttp = await Repository.GetAsync<List<EmployeeModel>>(url);
            if (responseHttp.Error)
            {
                var message = await responseHttp.GetErrorMessageAsync();
                Snackbar.Add(message!, Severity.Error);
                return new TableData<EmployeeModel> { Items = [], TotalItems = 0 };
            }
            ...
            return new TableData<EmployeeModel>
            {
        ...
        private async Task DeleteAsync(EmployeeModel employee)
        {
            var parameters = new DialogParameters
            {
            ...
            var responseHttp = await Repository.DeleteAsync<EmployeeModel>($"{baseUrl}/{employee.Id}");
            if (responseHttp.Error)
            {
